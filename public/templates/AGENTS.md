...
- Ship JS as native ES modules, built as Web Components for anything stateful/interactive. No bundler magic at runtime — see [Web Components Guideline](/style-guides/web-components/) for the full pattern.
...
- **BEM-shaped CSS class names** (`.block__element`, `.block--modifier`) — see [CSS Style Guide](/style-guides/css/), confirmed against Horizon's shipped CSS (native nesting and container queries are also used there and are fair game).
- **Web Components:** see [Web Components Guideline](/style-guides/web-components/) for the full pattern, including the optional `refs`-based declarative-event-binding approach Horizon's own components use, and when it's worth adopting versus a simpler plain-`HTMLElement` component.
- **New theme-wide brand colors: use the `color_palette` setting type, not separate `color` settings.** Shopify shipped this in the Spring '26 Edition (June 17, 2026) — a single shared, merchant-editable color grid that other `color`/`color_background` settings can default to via `{{ settings.<palette-id>.<key> }}`. As of this writing, Skeleton Theme (our scaffold) does not use it yet, but Shopify recommends it for new settings work and Horizon 4.0.0+ uses it throughout. See [Color Palettes](/design-system/color-palettes/) for the full mechanism and constraints (one per theme, `settings_schema.json` only, no `label`/`info`/`visible_if`). Don't migrate already-shipped separate `color` settings to this as a drive-by change — that's a deliberate, reviewed change on its own, not something to do incidentally while touching unrelated code.

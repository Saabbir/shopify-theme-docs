...
> the array the array should be filtered further before paginating.
...
#### section
Rendering a section with the `section` tag renders a section statically.
...
├── es.json                  # Spanish
├── es.schema.json           # Spanish
├── fr.json                  # French
├── fr.schema.json           # French
...
Everything above this line is Shopify's own generated content — produced by `shopify theme init` when AI agent support is selected, and identical to what any Shopify theme project starts from. **Don't edit anything above this line by hand** — if Shopify's CLI regenerates or updates that content in a future scaffold, reconcile by hand rather than assuming your edit survives a re-scaffold.

Everything below this line is Solis-specific — our project's own conventions, layered on top of Shopify's baseline. This is the section to edit when amending our rules; see [Managing & Amending AI Rules](/ai-assisted-development/managing-ai-rules/) for the full guideline on how.

### Source of truth & certainty requirements

These rules govern *how* every rule in this file — Shopify's content above, and ours below — gets applied. Read this as load-bearing, not optional context.

- **Official Shopify documentation is the only acceptable source for a factual claim about Shopify's platform, APIs, Liquid, or Theme Store policy.** Acceptable sources: `shopify.dev`, `help.shopify.com`, and Shopify's own official repos (`github.com/Shopify/*`). A third-party blog post, forum answer, or tutorial is not an acceptable source for a factual claim, even if convenient or plausible-sounding — it may be outdated, subtly wrong, or describe a deprecated pattern.
- **Do not guess about anything checkable.** If a Liquid object's properties, a filter's exact behavior, a schema field's valid values, or a Theme Store policy detail isn't something you're certain of, say so explicitly and check the official source before proceeding — do not present a guess as a fact. Call `learn_shopify_api` and use the Shopify AI Toolkit's doc search when available (see [Shopify's Official AI Toolkit](/ai-assisted-development/shopify-ai-toolkit/)); otherwise state the uncertainty and ask.
- **Plan before acting, for anything non-trivial.** State what you're about to build/change and why, before writing code — so a wrong assumption gets caught as a one-line correction instead of a rewrite. This is the same discipline the Figma-to-code workflow below applies specifically to design-to-code tasks; it applies more generally to any non-trivial change.
...
### Solis-specific constraints (in addition to Shopify's guidance above)
...
- Use CSS logical properties (`margin-inline-start`, `padding-inline-end`) instead of physical ones, so layout works correctly in RTL languages.
- Ship JS as native ES modules, built as Web Components for anything stateful/interactive. No bundler magic at runtime — see [Web Components: Two Patterns](/javascript/web-components-patterns/) for the full pattern.
- Don't add a dependency for something under ~50 lines of vanilla JS or native CSS can do — see [Third-Party Libraries](/style-guides/third-party-libraries/).
...
### Horizon-verified conventions

The conventions below were checked directly against Shopify's shipped Horizon and Skeleton Theme source — not just against Shopify's generic generated guidance above — because the generic guidance and the actual shipped code don't always agree. Where they differ, this states which one we follow and why.

- **LiquidDoc: use the dashed form, `{%- doc -%}` / `{%- enddoc -%}`.** Shopify's generated example above uses `{% doc %}` (no dashes) — both are valid Liquid, and since `{% doc %}` produces no rendered output the dashes have no functional effect either way. We standardize on the dashed form because that's what Horizon's actual snippets and blocks consistently ship.
- **Schema locale keys (`locales/*.schema.json`): flat, shared, purpose-based top-level namespaces — `settings.*`, `options.*`, `content.*`, `names.*`, `categories.*` — reused verbatim across every section/block, not nested per-component.** This differs from the hierarchical-per-feature key style in Shopify's generated example above (`products.card.description`); that's reasonable for the *storefront* locale file (`en.default.json`), but both Horizon's and Skeleton's actual `en.default.schema.json` use the flat, shared convention instead — the same label ("Background color," "Alignment") is reused by dozens of schemas, and nesting it per-component would duplicate near-identical strings dozens of times over. See [Complete Worked Example](/codebase-structure/complete-worked-example/) for a schema using this correctly.
- **Setting `id`s: `snake_case`, except a setting that mirrors a literal CSS custom property 1:1** (a padding/spacing control), which stays `kebab-case` to match the property it represents — e.g. `padding-block-start`, `padding-inline-end`. This is a narrow, specific exception, not general license for kebab-case setting IDs.
- **BEM-shaped CSS class names** (`.block__element`, `.block--modifier`) — see [CSS Architecture, Naming & Logical Properties](/css/css-architecture-naming-and-logical-properties/), confirmed against Horizon's shipped CSS (native nesting and container queries are also used there and are fair game).
- **Web Components:** see [Web Components: Two Patterns](/javascript/web-components-patterns/) for the full pattern, including the optional `refs`-based declarative-event-binding approach Horizon's own components use, and when it's worth adopting versus a simpler plain-`HTMLElement` component.
- **New theme-wide brand colors: use the `color_palette` setting type, not separate `color` settings.** Shopify shipped this in the Spring '26 Edition (June 17, 2026) — a single shared, merchant-editable color grid that other `color`/`color_background` settings can default to via `{{ settings.<palette-id>.<key> }}`. As of this writing, Skeleton Theme (our scaffold) does not use it yet, but Shopify recommends it for new settings work and Horizon 4.0.0+ uses it throughout. See [Color Palettes](/colors/color-palettes/) for the full mechanism and constraints (one per theme, `settings_schema.json` only, no `label`/`info`/`visible_if`). Don't migrate already-shipped separate `color` settings to this as a drive-by change — that's a deliberate, reviewed change on its own, not something to do incidentally while touching unrelated code.

### Figma-to-code workflow

When given a Figma frame/screenshot and asked to build a section, block, or snippet: **plan first** (decompose content vs. chrome, decide which artifact type it actually needs, list settings/blocks), **then build**, **then run `shopify theme check`**, **then fix any offenses**, **then report a short summary** of what was built and any decisions made. Full process: [Figma to Code Workflow](/ai-assisted-development/figma-to-code-workflow/). For the repeatable version of this exact loop, use the `/figma-to-liquid` custom command — see [Claude Code Custom Commands](/ai-assisted-development/claude-code-custom-commands/).

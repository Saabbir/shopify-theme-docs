...
- **The rules for `{% schema %}` blocks.** For example: one CSS property on its own becomes a custom property, but several properties together become a CSS class. This matches [CSS Style Guide](/style-guides/css/) exactly.
...
Because Shopify's file already explains all of this clearly and correctly, this handbook's own [Liquid Style Guide](/style-guides/liquid/), [CSS Style Guide](/style-guides/css/), and [Reference](/reference/) pages don't try to repeat it. Instead, they add the extra, day to day judgment calls on top.
